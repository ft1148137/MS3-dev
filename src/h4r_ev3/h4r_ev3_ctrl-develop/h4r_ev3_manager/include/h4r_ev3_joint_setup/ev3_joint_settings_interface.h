...
public:
	Ev3JointInterfaceHandle getHandle(const std::string& name)
	 ...
	 bool ControllerChange(const std::list<ControllerInfo> &start_list)
	 {
		...
		{   for(std::vector<InterfaceResources> ::const_iterator it_1 = (it->claimed_resources.begin()); it_1 != (it->claimed_resources.end()); it_1++ ){
				for (std::set<std::string>::const_iterator res = it_1->resources.begin(); res != it_1->resources.end(); res++)
				{
